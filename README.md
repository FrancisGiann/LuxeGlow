...
Supabase Auth, Postgres/RLS, Cloudinary image delivery and Edge Functions. The old PHP/MySQL
surface remains only as a rollback archive; it is not an active data path.

```
browser ──► React SPA ──► Supabase Auth + PostgREST/RPC
             │
             └── staff image Edge Function ──► Cloudinary

scheduler ──► scheduled Edge Function ──► email provider
```
