      ...
      <div className="mx-auto max-w-[1120px] px-5 sm:px-8 lg:px-12">
        <div className="border-b border-line pb-8 sm:pb-10">
          <div className="flex flex-wrap items-end justify-between gap-6">
            ...
            <Button type="button" size="lg" onClick={book}>{isStaff ? 'Open staff workspace' : 'Book an appointment'}</Button>
          </div>
        </div>

        <div className="py-8 sm:py-10">
          <ServiceCatalog services={services} loading={loading} error={error} onRetry={reload} showBookActions={!isStaff} />
        </div>
        ...
        {!loading && !error && services?.length > 0 && (
          <div className="flex flex-col gap-4 border-t border-line pt-7 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button type="button" variant="soft" size="lg" onClick={book}>{isStaff ? 'Open staff workspace' : 'Continue to booking'}</Button>
          </div>
        )}
      </div>
    </section>
