...
export function DashboardOverviewPage() {
  ...
  return <div className="mx-auto max-w-[900px]"><div className="mb-8"><h2 className="font-display text-3xl font-medium text-ink-900">Welcome back, {customer?.first_name || 'friend'}.</h2><p className="mt-2 text-ink-500">Your next visit and the actions you use most.</p></div><NextAppointment /><nav aria-label="Customer shortcuts" className="mt-6 flex flex-wrap items-center justify-center gap-3"><Link to="/book" className="rounded-xl bg-brand-800 px-6 py-3 text-sm font-bold text-white shadow-card hover:bg-brand-900">Book an appointment</Link><Link to="/dashboard/appointments" className="rounded-xl border border-line bg-surface px-6 py-3 text-sm font-bold text-brand-800 hover:border-brand-300">View appointments</Link></nav></div>;
}
