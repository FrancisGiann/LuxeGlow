  ...
  return (
    <Card className="overflow-hidden">
      <div className="flex items-center justify-between gap-4 border-b border-line px-6 py-5">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-500">Next appointment</p>
          <h2 className="mt-2 font-display text-2xl font-medium text-ink-900">{upcoming ? upcoming.service : 'Nothing scheduled yet'}</h2>
          {upcoming && <p className="mt-1 text-sm text-ink-500">{upcoming.staff_name || 'Unassigned'}</p>}
        </div>
        {upcoming && <StatusPill status={upcoming.status} />}
      </div>
      {upcoming ? (
        <div className="grid gap-5 px-6 py-6 sm:grid-cols-[1fr_auto] sm:items-end">
          <dl className="grid gap-3 text-sm sm:grid-cols-2">
            <div>
              <dt className="flex items-center gap-2 text-ink-500"><IconCalendar size={15} />Date</dt>
              <dd className="mt-1 font-semibold text-ink-900">{formatLongDate(toAppointmentDate(upcoming.raw_date, upcoming.raw_time))}</dd>
            </div>
            <div>
              <dt className="flex items-center gap-2 text-ink-500"><IconClock size={15} />Time</dt>
              <dd className="mt-1 font-semibold text-ink-900">{formatTime(toAppointmentDate(upcoming.raw_date, upcoming.raw_time))}</dd>
            </div>
          </dl>
          <p className="font-display text-2xl font-semibold text-brand-800">{formatPeso(upcoming.price)}</p>
        </div>
      ) : (
...
export function DashboardOverviewPage() {
  ...
  return (
    <div className="mx-auto max-w-[900px]">
      <div className="mb-8">
        <h2 className="font-display text-3xl font-medium text-ink-900">Welcome back, {customer?.first_name || 'friend'}.</h2>
        <p className="mt-2 text-ink-500">Your next visit and the actions you use most.</p>
      </div>
      <NextAppointment />
    </div>
  );
