...
import { IconCalendar, IconCheckCircle, IconClock, IconSparkle, IconStar } from '../components/icons';
...
function ServicePicker({ services, selectedIds, onToggle }) {
  ...
  return <div className="divide-y divide-line overflow-hidden rounded-xl border border-line">{services.map((service) => {
    const selected = selectedIds.includes(service.id);
    const image = assetUrl(service.image_path);
    return <label key={service.id} className={`flex min-h-[78px] cursor-pointer items-center gap-3 px-3 py-3 transition-colors sm:px-4 ${selected ? 'bg-brand-50' : 'bg-surface hover:bg-blush-50'} ${!selected && limitReached ? 'cursor-not-allowed opacity-55' : ''}`}>
      <input type="checkbox" checked={selected} disabled={!selected && limitReached} onChange={() => onToggle(service.id)} className="sr-only" />
      <span className={`relative h-14 w-14 shrink-0 overflow-hidden rounded-lg ${selected ? 'ring-2 ring-brand-600 ring-offset-2 ring-offset-brand-50' : 'bg-blush-100'}`}>
        {image ? <img src={image} alt="" className="h-full w-full object-cover" loading="lazy" /> : <span className="flex h-full items-center justify-center text-brand-600"><IconSparkle size={18} /></span>}
      </span>
      <span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-ink-900">{service.name}</span><span className="mt-1 flex items-center gap-2 text-xs text-ink-500"><IconClock size={13} />{service.duration}{service.rating > 0 && <span className="flex items-center gap-1 text-gold-600"><IconStar size={11} filled />{Number(service.rating).toFixed(1)}</span>}</span></span>
      <span className="flex shrink-0 items-center gap-3"><span className="font-display text-base font-semibold text-brand-800">{formatPeso(service.price)}</span><span aria-hidden="true" className={`flex h-6 w-6 items-center justify-center rounded-md border ${selected ? 'border-brand-800 bg-brand-800 text-white' : 'border-line-strong bg-surface text-transparent'}`}><IconCheckCircle size={15} /></span></span>
    </label>;
  })}</div>;
}
