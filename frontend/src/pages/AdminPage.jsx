...
function ProfileRow({ profile, onChange, onReset }) {
  ...
  return <li className="flex flex-wrap items-center justify-between gap-3 border-b border-line py-4 last:border-b-0"><div className="min-w-0"><p className="font-semibold text-ink-900">{profile.first_name} {profile.last_name}</p><p className="truncate text-xs text-ink-500">{profile.email}</p></div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${profile.is_active ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger'}`}>{profile.is_active ? 'Active' : 'Inactive'}</span><select value={profile.role} disabled={busy} onChange={(event) => update({ role: event.target.value })} className="min-h-9 rounded-lg border border-line bg-surface px-2 text-xs font-semibold" aria-label={`Role for ${profile.first_name} ${profile.last_name}`}><option value="staff">Staff</option><option value="admin">Admin</option></select><Button type="button" size="sm" variant={profile.accepts_appointments ? 'primary' : 'soft'} disabled={busy} onClick={() => update({ accepts_appointments: !profile.accepts_appointments })} aria-pressed={profile.accepts_appointments} aria-label={`${profile.accepts_appointments ? 'Disable' : 'Enable'} appointments for ${profile.first_name} ${profile.last_name}`}>{profile.accepts_appointments ? 'Accepts appointments' : 'Does not accept appointments'}</Button><Button type="button" size="sm" variant="soft" disabled={busy} onClick={() => update({ is_active: !profile.is_active })}>{profile.is_active ? 'Deactivate' : 'Activate'}</Button><Button type="button" size="sm" variant="soft" disabled={busy} onClick={() => onReset(profile)}>Reset password</Button></div></li>;
}
