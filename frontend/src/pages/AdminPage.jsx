...
import { adminNavGroupForTab, appointmentFilterPreset, appointmentQueueEmptyMessage, createInitialAppointmentFilters, getAdminAppointmentQueue, initialAdminTab, manilaDateKey } from '../utils/adminAppointments';
...
const NAV_GROUPS = [
  { id: 'daily-work', label: 'Daily work', items: [
    ['overview', 'Overview', IconGrid],
    ['appointments', 'Appointments', IconCalendar],
    ['customers', 'Customers', IconUser],
  ] },
  { id: 'salon-setup', label: 'Salon setup', items: [
    ['catalog', 'Services', IconSparkle],
    ['schedule', 'Hours & closures', IconClock],
  ] },
  { id: 'website-content', label: 'Website content', items: [
    ['faqs', 'FAQs', IconCheckCircle],
    ['about', 'Business info', IconGrid],
  ] },
  { id: 'people-access', label: 'People & access', items: [
    ['staff', 'Staff accounts', IconUser],
  ], adminOnly: true },
];
...
const ADMIN_MOBILE_MENU_ID = 'admin-mobile-menu';

function AdminWorkspaceBrand() {
  return <div className="flex items-center gap-3 border-b border-line px-5 py-7"><span className="flex h-10 w-10 items-center justify-center rounded-full border border-gold-500 font-display text-xl font-medium text-gold-600">A</span><span className="font-display text-sm font-semibold leading-[0.95] text-ink-900">Astrid Nails<br /><span className="font-sans text-[8px] font-bold uppercase tracking-[0.15em] text-ink-500">&amp; Beauty Bar</span></span></div>;
}

function AdminNav({ tab, setTab, isAdmin, onNavigate }) {
  const groups = NAV_GROUPS.filter((group) => !group.adminOnly || isAdmin);
  const [openGroup, setOpenGroup] = useState(() => adminNavGroupForTab(tab) || 'daily-work');
  useEffect(() => {
    setOpenGroup(adminNavGroupForTab(tab));
  }, [tab]);
  const idPrefix = useId();
  return <nav aria-label="Staff sections" className="flex flex-col gap-2 px-3 py-6">{groups.map((group) => {
    const expanded = openGroup === group.id;
    const activeItem = group.items.find(([key]) => key === tab);
    const panelId = idPrefix + "-group-" + group.id;
    return <section key={group.id}>
      <button type="button" aria-expanded={expanded} aria-controls={panelId} onClick={() => setOpenGroup((current) => current === group.id ? null : group.id)} className="flex min-h-12 w-full items-center gap-3 rounded-lg px-3 text-left text-sm font-bold text-ink-700 transition-colors hover:bg-canvas focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-800">
        <span className="min-w-0 flex-1">{group.label}{activeItem && !expanded && <span className="mt-0.5 block truncate text-xs font-medium text-brand-800">Current: {activeItem[1]}</span>}</span>
        <IconArrowRight size={15} aria-hidden="true" className={"shrink-0 text-ink-500 transition-transform " + (expanded ? "rotate-90" : "")} />
      </button>
      <div id={panelId} hidden={!expanded} className="mt-1 flex flex-col gap-1">
        {group.items.map(([key, label, Icon]) => <button key={key} type="button" onClick={() => { setTab(key); setOpenGroup(adminNavGroupForTab(key)); onNavigate?.(); }} aria-current={tab === key ? 'page' : undefined} className={"relative flex min-h-11 items-center gap-3 rounded-lg px-3 text-left text-sm transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-800 " + (tab === key ? 'bg-blush-100 font-bold text-brand-800' : 'font-medium text-ink-600 hover:bg-canvas hover:text-ink-900')}><Icon size={18} />{label}</button>)}
      </div>
    </section>;
  })}</nav>;
}

function AdminWorkspaceFooter({ onLogout, onNavigate }) {
  return <div className="mt-auto border-t border-line p-4"><p className="text-xs text-ink-500">Staff workspace</p><Link to="/" onClick={onNavigate} className="mt-3 flex min-h-11 items-center gap-2 rounded-lg px-3 text-sm font-semibold text-brand-800 hover:bg-brand-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-800"><IconArrowRight size={16} className="rotate-180" />Back to homepage</Link><button type="button" onClick={() => { onLogout(); onNavigate?.(); }} className="mt-1 flex min-h-11 w-full items-center rounded-lg px-3 text-sm font-semibold text-danger hover:bg-blush-50">Log out</button></div>;
}

function AdminRail({ tab, setTab, isAdmin, onLogout }) {
  return <aside className="hidden border-r border-line bg-surface lg:block"><div className="sticky top-0 flex h-screen w-[210px] flex-col"><AdminWorkspaceBrand /><AdminNav tab={tab} setTab={setTab} isAdmin={isAdmin} /><AdminWorkspaceFooter onLogout={onLogout} /></div></aside>;
}

function AdminMobileDrawer({ open, onClose, tab, setTab, isAdmin, onLogout, pageTitle }) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      const focusable = drawer ? [...drawer.querySelectorAll('button:not([disabled]), a[href]')] : [];
  ...
  if (!open) return null;
  return <div id={ADMIN_MOBILE_MENU_ID} className="fixed inset-0 z-[1050] lg:hidden"><button type="button" className="absolute inset-0 h-full w-full cursor-default border-0 bg-ink-900/50" onClick={onClose} aria-label="Close staff navigation" /><aside role="dialog" aria-modal="true" aria-labelledby="admin-mobile-menu-title" className="absolute inset-y-0 left-0 flex w-[320px] max-w-[calc(100vw-1rem)] flex-col bg-surface shadow-pop"><div className="flex items-start justify-between gap-4"><div className="min-w-0 flex-1"><AdminWorkspaceBrand /></div><button ref={closeButtonRef} type="button" onClick={onClose} aria-label="Close staff navigation" className="mr-3 mt-3 flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-ink-500 hover:bg-canvas hover:text-ink-900"><IconX size={19} /></button></div><div className="border-b border-line px-5 py-4"><h2 id="admin-mobile-menu-title" className="font-display text-lg font-medium text-ink-900">Staff navigation</h2><p className="mt-1 text-sm text-ink-600">Current section: <span className="font-semibold text-ink-900">{pageTitle}</span></p></div><div className="min-h-0 flex-1 overflow-y-auto"><AdminNav tab={tab} setTab={setTab} isAdmin={isAdmin} onNavigate={onClose} /></div><AdminWorkspaceFooter onLogout={onLogout} onNavigate={onClose} /></aside></div>;
}
...
function AppointmentRow({ appointment, selected, onSelect }) {
  ...
  return <li className="border-b border-line last:border-b-0"><button type="button" onClick={() => onSelect(appointment)} className={`w-full block py-4 text-left leading-relaxed transition-colors focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-brand-800 ${selected ? 'bg-blush-50' : 'hover:bg-canvas'}`}><div className="grid min-h-20 items-center gap-3 sm:grid-cols-[0.9fr_1.3fr_minmax(0,1fr)_auto]"><div className="flex justify-between sm:contents"><span className="min-w-0"><span className="block font-display text-base font-medium leading-snug text-ink-900">{appointment.local_time?.slice(0, 5) || '—'}</span><span className="block text-xs leading-snug text-ink-500">{appointment.local_date || 'Date unavailable'}</span></span><span className="sm:hidden"><StatusPill status={appointment.status} size="sm" /></span></div><span className="min-w-0 overflow-hidden"><span className="block break-words text-sm font-bold leading-snug text-ink-900">{customer.first_name} {customer.last_name}</span><span className="block break-all text-xs leading-snug text-ink-500">{customer.email || 'No email'}</span><span className="block break-words text-xs leading-snug text-ink-500">{appointmentStaffName(appointment)}</span></span><span className="min-w-0 break-words text-sm leading-snug text-ink-600">{serviceNames(appointment).join(', ') || 'No services'}</span><span className="hidden justify-self-end whitespace-nowrap sm:block"><StatusPill status={appointment.status} size="sm" /></span></div></button></li>;
}

function AppointmentDetails({ appointment, onRequestStatus, onReschedule, onViewInAppointments }) {
  const customer = appointment.customer || {};
  const terminal = isTerminalAppointment(appointment);
  return <>
    <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-500">Appointment details</p><h2 className="mt-2 break-all font-display text-2xl font-medium text-ink-900">{appointment.reference_no || 'Reference unavailable'}</h2></div><StatusPill status={appointment.status} /></div>
    <div className="mt-6 border-t border-line pt-5"><h3 className="font-sans text-xs font-bold uppercase tracking-[0.16em] text-ink-500">Customer</h3><p className="mt-3 font-semibold text-ink-900">{[customer.first_name, customer.last_name].filter(Boolean).join(' ') || 'Customer details unavailable'}</p><p className="mt-1 break-all text-sm text-ink-600">{customer.email || 'No email'}</p>{customer.phone && <p className="mt-1 text-sm text-ink-600">{customer.phone}</p>}</div>
    <div className="mt-6 border-t border-line pt-5"><h3 className="font-sans text-xs font-bold uppercase tracking-[0.16em] text-ink-500">When</h3><p className="mt-3 flex items-center gap-2 text-sm font-semibold text-ink-900"><IconCalendar size={16} className="text-brand-600" />{localDateLabel(appointment.local_date)}</p><p className="mt-2 flex items-center gap-2 text-sm font-semibold text-ink-900"><IconClock size={16} className="text-brand-600" />{appointment.local_time?.slice(0, 5) || 'Time unavailable'} · {appointment.total_duration_minutes || '—'} min</p><p className="mt-2 text-sm text-ink-600">Team member: <span className="font-semibold text-ink-900">{appointmentStaffName(appointment)}</span></p>{appointment.staff_id && appointment.staff_rating_count > 0 && <p className="mt-2 text-sm text-ink-600">Published staff rating: <span className="font-semibold text-ink-900">{Number(appointment.staff_average_rating).toFixed(1)} / 5</span> <span className="text-xs text-ink-500">({appointment.staff_rating_count} visit{appointment.staff_rating_count === 1 ? '' : 's'})</span></p>}</div>
    <div className="mt-6 border-t border-line pt-5"><h3 className="font-sans text-xs font-bold uppercase tracking-[0.16em] text-ink-500">Services</h3>{appointment.services?.length ? <ul className="mt-3 divide-y divide-line">{appointment.services.map((service, index) => <li key={`${appointment.id}-${service.service_name}-${index}`} className="flex justify-between gap-3 py-2 text-sm"><span className="min-w-0 text-ink-700">{service.service_name || 'Unnamed service'}</span><span className="shrink-0 font-semibold text-ink-900">{formatPeso(service.unit_price)}</span></li>)}</ul> : <p className="mt-3 text-sm text-ink-500">No services recorded.</p>}<p className="mt-3 flex justify-between border-t border-line pt-3 text-sm font-bold"><span>Total</span><span className="font-display text-lg text-brand-800">{formatPeso(appointment.total_price)}</span></p></div>
    {terminal ? <p className="mt-6 border-t border-line pt-5 text-sm leading-relaxed text-ink-500">Completed and cancelled appointments are read-only.</p> : <div className="mt-6 flex flex-wrap gap-2 border-t border-line pt-5"><Button type="button" size="sm" variant="soft" className="min-h-11" onClick={() => onReschedule(appointment)}>Reschedule</Button>{appointmentActions(appointment.status).map((action) => <Button key={action.status} type="button" size="sm" variant={action.variant} className="min-h-11" onClick={() => onRequestStatus(appointment, action.status)}>{action.label}</Button>)}</div>}
    {onViewInAppointments && <div className="mt-5 border-t border-line pt-5"><Button type="button" size="sm" variant="soft" className="min-h-11 w-full sm:w-auto" onClick={onViewInAppointments}>View in appointments</Button></div>}
  </>;
}
...
function AppointmentInspector({ appointment, onRequestStatus, onReschedule }) {
  ...
  return <Card className="h-full p-6"><AppointmentDetails appointment={appointment} onRequestStatus={onRequestStatus} onReschedule={onReschedule} /></Card>;
}

function AppointmentQueue({ appointments, filters, totalAppointments, selectedAppointment, onFilterChange, onAllDates, onReset, onSelectAppointment }) {
  return <Card className="overflow-hidden p-5 sm:p-7">
    <CardHeader title="Appointments" subtitle="Search and filter booking requests. Dates and times use Asia/Manila." />
    <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(14rem,1.4fr)_minmax(10rem,0.8fr)_minmax(10rem,0.8fr)]">
      <label className="text-sm font-semibold text-ink-900"><span className="invisible">Search appointments</span><span className="relative block mt-1"><IconSearch size={17} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-500" /><input type="search" value={filters.search} onChange={(event) => onFilterChange('search', event.target.value)} placeholder="Search appointments" className="min-h-11 w-full rounded-lg border border-line bg-surface px-3 pl-10 text-sm focus:border-brand-500 focus:outline-none focus:ring-4 focus:ring-brand-100" /></span></label>
      <label className="text-sm font-semibold text-ink-900">Date<input type="date" value={filters.date} onChange={(event) => onFilterChange('date', event.target.value)} className="mt-1 block min-h-11 w-full rounded-lg border border-line bg-surface px-3 text-sm focus:border-brand-500 focus:outline-none focus:ring-4 focus:ring-brand-100" /></label>
      <label className="text-sm font-semibold text-ink-900">Status<select value={filters.status} onChange={(event) => onFilterChange('status', event.target.value)} className="mt-1 block min-h-11 w-full rounded-lg border border-line bg-surface px-3 text-sm focus:border-brand-500 focus:outline-none focus:ring-4 focus:ring-brand-100"><option value="all">All statuses</option>{['Pending', 'Confirmed', 'Completed', 'Cancelled'].map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
    </div>
    <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-b border-line pb-4">
      <p className="text-sm font-semibold text-ink-600" role="status" aria-live="polite">{appointments.length} of {totalAppointments} appointment{totalAppointments === 1 ? '' : 's'}</p>
      <div className="flex flex-wrap items-center gap-2">
        <button type="button" onClick={onAllDates} className="min-h-11 rounded-lg px-3 text-sm font-semibold text-brand-800 underline decoration-line underline-offset-4 hover:text-brand-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-800">All dates</button>
        <button type="button" onClick={onReset} className="min-h-11 rounded-lg border border-line bg-surface px-3 text-sm font-semibold text-ink-700 hover:border-brand-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-800">Reset to today</button>
      </div>
    </div>
    <div className="mt-3 hidden min-h-12 items-center gap-2 border-b border-line bg-canvas py-3 text-xs font-bold uppercase leading-snug tracking-[0.12em] text-ink-500 sm:grid sm:grid-cols-[0.9fr_1.3fr_1fr_auto]"><span>Time</span><span>Customer</span><span>Services</span><span className="sm:text-right">Status</span></div>
    <ul className="mt-1">{appointments.length ? appointments.map((appointment) => <AppointmentRow key={appointment.id} appointment={appointment} selected={selectedAppointment?.id === appointment.id} onSelect={onSelectAppointment} />) : <li className="py-12 text-center text-sm text-ink-500">{appointmentQueueEmptyMessage(totalAppointments, filters)}</li>}</ul>
  </Card>;
}

function NotificationAppointmentDialog({ state, appointment, onClose, onRequestStatus, onReschedule, onViewInAppointments }) {
  if (!state) return null;
  return <AdminDialog open title="Appointment details" description={appointment ? 'Review the latest appointment details and available actions.' : 'The appointment linked to this notification could not be displayed.'} onClose={onClose}>
    {state.loading && <div className="flex items-center gap-3 py-8 text-sm text-ink-500" role="status"><Spinner size="sm" tone="brand" />Loading the latest appointment details…</div>}
    {!state.loading && state.error && <p className="rounded-lg border border-danger/30 bg-danger/10 px-3 py-2 text-sm font-semibold text-danger" role="alert">{state.error}</p>}
    {!state.loading && appointment && <AppointmentDetails appointment={appointment} onRequestStatus={onRequestStatus} onReschedule={onReschedule} onViewInAppointments={onViewInAppointments} />}
  </AdminDialog>;
}
...
function AdminOverview({ appointments, onOpenAppointments, onOpenAppointment }) {
  ...
  return <div className="space-y-6"><div><p className="text-sm text-ink-500">A live operational view for {localDateLabel(today)}. Appointment totals are not payment or revenue figures.</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => onOpenAppointments('today')} className="rounded-xl border border-line bg-surface p-5 text-left transition-colors hover:border-brand-300 hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-800"><span className="block text-sm font-semibold text-ink-600">Today’s bookings</span><span className="mt-2 block font-display text-3xl font-medium text-brand-800">{todayBookings.length}</span><span className="mt-1 block text-xs text-ink-500">Lucena City local date · Open today’s queue</span></button><button type="button" onClick={() => onOpenAppointments('pending')} className="rounded-xl border border-line bg-surface p-5 text-left transition-colors hover:border-brand-300 hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-800"><span className="block text-sm font-semibold text-ink-600">Pending requests</span><span className="mt-2 block font-display text-3xl font-medium text-brand-800">{pendingRequests.length}</span><span className="mt-1 block text-xs text-ink-500">Awaiting staff action · Review all pending</span></button></div><dl className="mt-3 grid grid-cols-1 gap-2 text-sm sm:grid-cols-2"><div className="min-w-0 rounded-xl border border-line bg-surface p-3"><dt className="break-words text-ink-500">Bookings this month</dt><dd className="mt-1 break-words font-bold tabular-nums text-ink-800">{monthBookings.length}</dd></div><div className="min-w-0 rounded-xl border border-line bg-surface p-3"><dt className="break-words text-ink-500">Weekend bookings this month</dt><dd className="mt-1 break-words font-bold tabular-nums text-ink-800">{weekendBookings.length}</dd></div></dl></div><div className="grid gap-6 xl:grid-cols-2"><Card className="p-5 sm:p-7"><CardHeader title="Recent bookings" subtitle="Latest appointment requests and their service snapshots." />{recentBookings.length ? <ul className="mt-2 divide-y divide-line">{recentBookings.map((appointment) => <li key={appointment.id}><button type="button" onClick={() => onOpenAppointment(appointment)} className="flex min-h-16 w-full items-center justify-between gap-4 py-3 text-left hover:bg-canvas focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-800"><span className="min-w-0"><span className="block break-words text-sm font-bold text-ink-900">{appointment.reference_no}</span><span className="block text-xs leading-relaxed text-ink-500"><span className="block sm:inline">{localDateLabel(appointment.local_date)} · {appointment.local_time?.slice(0, 5) || 'Time unavailable'}</span> <span className="hidden sm:inline">·</span> <span className="block sm:inline">{appointmentStaffName(appointment)} · {serviceNames(appointment).join(', ') || 'No services'}</span></span></span><StatusPill status={appointment.status} size="sm" /></button></li>)}</ul> : <p className="py-8 text-sm text-ink-500">No bookings yet.</p>}</Card><Card className="p-5 sm:p-7"><CardHeader title="Popular services this month" subtitle="Counted from appointment service snapshots; cancelled bookings excluded." />{popularServices.length ? <ol className="mt-2 divide-y divide-line">{popularServices.map(([name, count]) => <li key={name} className="flex items-center justify-between gap-4 py-3"><span className="min-w-0 truncate text-sm font-semibold text-ink-800">{name}</span><span className="shrink-0 text-sm font-bold text-brand-800">{count} booking{count === 1 ? '' : 's'}</span></li>)}</ol> : <p className="py-8 text-sm text-ink-500">No non-cancelled bookings this month.</p>}</Card></div></div>;
}
...
export function AdminPage() {
  ...
  const [notificationAppointmentModal, setNotificationAppointmentModal] = useState(null);
  ...
  const [appointmentFilters, setAppointmentFilters] = useState(() => createInitialAppointmentFilters());
  ...
  const openAppointments = useCallback((kind = 'today', appointment = null) => {
    setAppointmentFilters(appointmentFilterPreset(kind, manilaDateKey(), appointment));
    setSelectedAppointment(kind === 'appointment' ? appointment : null);
    setTab('appointments');
  }, []);
  const visibleAppointments = useMemo(() => getAdminAppointmentQueue(appointments, appointmentFilters), [appointments, appointmentFilters]);
  useEffect(() => {
    if (selectedAppointment && !visibleAppointments.some((appointment) => appointment?.id === selectedAppointment.id)) setSelectedAppointment(null);
  ...
  const load = useCallback(async (initialLoad = false) => {
    ...
    try {
      ...
      setNotificationAppointmentModal((current) => {
        if (!current) return current;
        const refreshedAppointment = nextAppointments.find((appointment) => String(appointment.id) === current.appointmentId);
        return refreshedAppointment
          ? { ...current, appointment: refreshedAppointment, error: '' }
          : { ...current, appointment: null, loading: false, error: 'This appointment is no longer available. It may have been deleted or you may no longer have access.' };
      });
  ...
  const closeNotificationAppointment = useCallback(() => {
    ...
    setNotificationAppointmentModal(null);
  }, []);

  const openNotificationAppointment = useCallback(async (notification) => {
    ...
    const currentAppointment = appointments.find((appointment) => String(appointment.id) === appointmentId) || null;
    openAppointments('appointment', currentAppointment);
    setNotificationAppointmentModal({ appointmentId, appointment: currentAppointment, loading: true, error: '' });
    if (!appointmentId) {
      setNotificationAppointmentModal({ appointmentId, appointment: null, loading: false, error: 'This notification is not linked to an appointment.' });
      return;
    }
    try {
      const nextAppointments = await listAdminAppointments();
      if (notificationAppointmentRequestRef.current !== requestId) return;
      const refreshedAppointments = Array.isArray(nextAppointments) ? nextAppointments : [];
      setAppointments(refreshedAppointments);
      const refreshedAppointment = refreshedAppointments.find((appointment) => String(appointment.id) === appointmentId) || null;
      openAppointments('appointment', refreshedAppointment);
      setNotificationAppointmentModal({
        appointmentId,
        appointment: refreshedAppointment,
        loading: false,
        error: refreshedAppointment ? '' : 'This appointment is no longer available. It may have been deleted or you may no longer have access.',
      });
      ...
      setNotificationAppointmentModal({
        ...
        loading: false,
        error: currentAppointment
          ? `Could not refresh this appointment. Showing the last loaded details. ${appointmentError?.message || ''}`.trim()
          : appointmentError?.message || 'Could not load this appointment. It may have been deleted or you may no longer have access.',
      });
    }
  ...
  const requestNotificationStatus = (appointment, nextStatus) => {
    closeNotificationAppointment();
    requestStatus(appointment, nextStatus);
  };
  const rescheduleNotificationAppointment = (appointment) => {
    closeNotificationAppointment();
