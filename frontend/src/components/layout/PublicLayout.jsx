    ...
    <header className={`fixed inset-x-0 top-0 z-[900] border-b border-line bg-canvas/95 transition-shadow duration-300 ${scrolled ? 'shadow-card backdrop-blur-sm' : ''}`}>
      <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between gap-6 px-5 sm:px-8 lg:px-14">
        ...
        <div className="hidden items-center gap-3 xl:flex">
          ...
          {!isStaff && <button type="button" onClick={() => navigate('/book')} className="min-h-11 rounded-xl bg-brand-800 px-5 text-sm font-bold text-white shadow-card hover:bg-brand-900">Book</button>}
        </div>
...
export function PublicLayout() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (returnPath === '/book') window.sessionStorage.removeItem('luxeglow-auth-return');
    } catch {
      return;
    }
